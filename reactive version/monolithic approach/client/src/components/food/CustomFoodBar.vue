    ...
    <div class="bg-white border border-gray-300 px-4 py-3 rounded w-1/2 h-1/2">
      ...
      <div class="flex items-center justify-between gap-2 mb-4">
        ...
        <div class="flex gap-2">
          <Avatar icon="pi pi-angle-left"
                  class="cursor-pointer p-1 m-1 bg-gray-200 border border-gray-300 rounded-full hover:bg-gray-300 transition-colors duration-200"
                  v-tooltip.top="'Prev'"
                  @click="emits('navigate', 'left')"/>
          <Avatar icon="pi pi-angle-right"
                  v-tooltip.top="'Next'"
                  class="cursor-pointer p-1 m-1 bg-gray-200 border border-gray-300 rounded-full hover:bg-gray-300 transition-colors duration-200"
                  @click="emits('navigate', 'right')"/>
        </div>
      </div>
