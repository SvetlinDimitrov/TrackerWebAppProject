...
import org.trackerwebapp.trackerwebapp.domain.dto.nutritionxApi.FullNutrient;
...
public class NutrientMapperUtils {
  ...
  public static List<NutritionView> getNutrients(FoodItem dto) {
    ...
    return nutrients
        ...
        .map(data -> new NutritionView(data.name() , data.unit() , data.amount().setScale(2 , RoundingMode.HALF_EVEN)))
        .toList();
  }
    ...
    Optional.ofNullable(dto)
        ...
        .flatMap(data -> data
          ...
          NutritionView nutrient = new NutritionView(
              ...
              value.getValue().multiply(BigDecimal.valueOf(1000)).setScale(2, RoundingMode.DOWN)
          );
          list.add(nutrient);
        });
  }
...
/**
 * vitamin_values = {
 * "Vitamin A": nutrient_table["VITA_RAE"],
 * "Vitamin D (D2 + D3)": nutrient_table["VITD"],
 * "Vitamin E": nutrient_table["TOCPHA"],
 * "Vitamin K": nutrient_table["VITK1"],
 * "Vitamin C": nutrient_table["VITC"],
 * "Vitamin B1 (Thiamin)": nutrient_table["THIA"],
 * "Vitamin B2 (Riboflavin)": nutrient_table["RIBF"],
 * "Vitamin B3 (Niacin)": nutrient_table["NIA"],
 * "Vitamin B5 (Pantothenic acid)": nutrient_table["PANTAC"],
 * "Vitamin B6": nutrient_table["VITB6A"],
 * "Vitamin B7 (Biotin)": nutrient_table["VITB7"],
 * "Vitamin B9 (Folate)": nutrient_table["FOL"],
 * "Vitamin B12": nutrient_table["VITB12"],
 * "Choline": nutrient_table["CHOLN"]
 * }
 **/
