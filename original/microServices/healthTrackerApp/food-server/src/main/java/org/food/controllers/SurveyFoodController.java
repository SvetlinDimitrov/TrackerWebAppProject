...
@Tag(name = "Survey Food", description = "The Survey Food API provides operations for interacting with survey foods.")
public interface SurveyFoodController {
    ...
    @Operation(summary = "Get final food by id", responses = {
            @ApiResponse(responseCode = "200", description = "Successful operation",
                    content = @Content(schema = @Schema(implementation = SurveyFoodView.class))),
            @ApiResponse(responseCode = "400", description = "Food Exception",
                    content = @Content(schema = @Schema(implementation = FoodException.class)))})
    @GetMapping(path = "/{id}")
    SurveyFoodView getFinalFoodById(
    ...
    public SurveyFoodView calculateFood(
            @RequestBody SurveyFoodView food,
